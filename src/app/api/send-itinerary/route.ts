  ...
  try {
    ...
    const emailHtml = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #334155; padding: 20px; border: 1px solid #e2e8f0; border-radius: 16px;">
        ...
        <p>Hi there,</p>
        <p>Here is your complete, customized travel itinerary for <b>${destination}</b>!</p>
    ...
    // 1. Create SMTP transporter using Gmail details
    const transporter = nodemailer.createTransport({
      service: "gmail",
      auth: {
        user: process.env.EMAIL_USER || "singhaditya98845@gmail.com",
        pass: process.env.EMAIL_APP_PASSWORD || "kejxdmygmfkjqydm"
      }
    });

    // 2. Send email via transporter
    await transporter.sendMail({
      from: `"WanderAI" <${process.env.EMAIL_USER || "singhaditya98845@gmail.com"}>`,
      to: userEmail,
    ...
    console.error("Error emailing itinerary via SMTP:", error);
